...
完了前に以下を自己チェックする（今セッションで繰り返し出た指摘）:

| 観点 | チェック内容 |
|------|------------|
| 型安全 | `as` キャストを使っていないか。外部 API レスポンスは型ガード関数（`isXxx`）で検証しているか |
| 非同期 | `useEffect` 内の fetch は `AbortController` + `ignore` フラグで race condition を防いでいるか。クリーンアップ関数があるか |
| アクセシビリティ | `input` / `button` / `select` に `aria-label` を付けているか |
| 二重送信防止 | 非同期操作中は `disabled` で操作を封じているか |
| 入力検証 | API のクエリパラメータは正規表現 + 範囲チェックで検証しているか |
