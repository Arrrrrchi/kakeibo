...
- Docker Compose で PostgreSQL を起動する（`docker-compose.yml` を作成）
...
- `upsertMany`: `createMany` + `skipDuplicates` で `import_hash` による重複排除つき一括挿入
- `getMonthlyAggregation`: `$queryRaw` で `to_char(date, 'YYYY-MM')` によるグルーピング。収入・支出を `is_income` で分離
- `getCategoryBreakdown`: `$queryRaw` で `major_category`, `minor_category` によるグルーピング。支出のみ（`is_income: false`）
- `findByCategory`: `findMany` で指定カテゴリの取引一覧を日付降順で取得
- `getDistinctCategories`: `$queryRaw` で支出取引の `major_category` + `minor_category` のユニーク一覧
- `getMonthlyTrendByCategory`: `$queryRaw` で特定カテゴリの月別合計（モーダル表示用）
...
- 入力: パース済み CSV データの配列（`TransactionCreateInput[]`）
- 処理: `TransactionRepository.upsertMany()` を呼び出し
  - ハッシュ生成・フィルタリング・金額変換は CSV パーサー側で実行済み
- 出力: 挿入件数
...
- エンコーディングの自動判定
  - まず `TextDecoder("utf-8", { fatal: true })` で UTF-8 を試行
  - 失敗時に `TextDecoder("shift-jis")` で cp932 にフォールバック
- CSV パース処理
  - ヘッダー行を解析してカラム位置を特定
...
export async function parseMoneyforwardCsv(
  buffer: Buffer
): Promise<TransactionCreateInput[]> {
  // 1. UTF-8 優先、失敗時に cp932 フォールバック
...
`import "server-only"` を使用するモジュール（CSV パーサー、リポジトリ実装）をテストするため、以下の対応を行う:

- `src/test/helpers/server-only-mock.ts` に空のモジュールを作成
- `vitest.config.ts` の `resolve.alias` で `server-only` をモックに差し替え
...
- [x] `pnpm exec prisma migrate dev` が正常に完了する
- [x] Prisma クライアントシングルトンが正常に動作する
- [x] 全リポジトリの CRUD 操作が正常に動作する
- [x] ユースケースが想定通りのデータを返す
- [x] CSV パーサーがサンプル CSV を正しくパースできる
- [x] cp932 エンコーディングの日本語が文字化けしない
- [x] 重複 CSV インポート時に `import_hash` で正しく重複排除される
- [x] TypeScript 型チェックがエラーなく通過する
- [x] `pnpm test:run` で全テスト（31 件）が通過する
- [x] CSV パーサーのテスト: 正常系・フィルタリング・エッジケースが通過（10 件）
- [x] ユースケースのテスト: KPI 計算・未割当算出・予算対比が通過（14 件）
- [x] フォーマット関数のテスト: 全パターンが通過（6 件）

## 成果物

| ファイル | 説明 |
|---|---|
| `docker-compose.yml` | PostgreSQL の Docker Compose 設定 |
| `prisma/migrations/20260228132245_init/` | 初回マイグレーション |
| `src/server/lib/csv-parser.ts` | マネーフォワード CSV パーサー |
| `src/client/lib/format.ts` | 通貨・パーセント・月フォーマット関数 |
...
| `src/server/lib/csv-parser.test.ts` | CSV パーサーテスト（10 件） |
| `src/server/usecases/*.test.ts` | ユースケーステスト（5 ファイル、14 件） |
| `src/client/lib/format.test.ts` | フォーマット関数テスト（6 件） |
| `src/test/helpers/mock-repositories.ts` | リポジトリモックヘルパー |
| `src/test/helpers/server-only-mock.ts` | server-only モック |
| `src/test/fixtures/sample.csv` | テスト用サンプル CSV（cp932） |
